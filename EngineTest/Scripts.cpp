...
public:
	...
	void update(f32 dt) override
	{
		...
		if (_angle > math::two_pi) _angle -= math::two_pi;
		math::v3a rot{ _angle, 0.f, math::pi/2 };
		DirectX::XMVECTOR quat{ DirectX::XMQuaternionRotationRollPitchYawFromVector(DirectX::XMLoadFloat3A(&rot)) };
		math::v4 rot_quat{};
		DirectX::XMStoreFloat4(&rot_quat, quat);
		set_rotation(rot_quat);
...
public:
	...
	void update(f32 dt) override
	{
		...
		pos.y = 0.8f + 0.2f * std::sin(x) * std::sin(std::sin(x / 1.62f) + std::sin(1.62f * x) + std::sin(3.24f * x));
		set_position(pos);
	}
...
public:
		...
		:script::entity_script{ entity }
	{
		...
		_desired_spherical = DirectX::XMLoadFloat3(&rot);
	}
	...
	void update(f32 dt) override
	{
		...
		const static u64 binding{ std::hash<std::string>()("move") };
