...
// If you get any compilation or linker errors then make sure that
// 1) FBX SDK 2020.2 or later is installed on your system
// 2) the include path to fbxsdk.h is added to "Addtional Include Directories" (compiler settings)
// 3) the library paths in the following section point to the correc location
...
namespace primal::tools
{
	...
	void fbx_context::get_scene(FbxNode* root /*= nullptr*/)
	{
		...
		for (s32 i{ 0 }; i < num_nodes; ++i)
		{
			...
			lod_group lod{};
			get_meshes(root, lod.meshes, 0, -1.f);
			if (lod.meshes.size())
			{
	...
	bool fbx_context::get_mesh_data(FbxMesh* fbx_mesh, mesh& m)
	{
		...
		for (s32 i{ 0 }; i < num_indices; ++i)
		{
			...
			// 이 정점을 만난적이 있으면 그냥 이 정점의 index만 추가
			// 아니라면 그 정점과 새로운 인덱스 추가
		...
		if (import_normals)
		{
			...
			{
				// 만약 법선벡터를 불러오는데 실패했다면 법선벡터를 직접 계산한다.
				// somrthing went wrong with importing normals from FBX.
				// Fall back to our own normal calcuation method.
		...
		if (import_tangents)
		{
			...
			{
				// 만약 법선벡터를 불러오는데 실패했다면 접선벡터를 직접 계산한다.
				// somrthing went wrong with importing normals from FBX.
				// Fall back to our own normal calcuation method.
