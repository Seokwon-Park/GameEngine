...
namespace PrimalEditor.Components
{
    ...
    class GameEntity : ViewModelBase
    {
        ...
        {
            Debug.Assert(component != null);
            if (component is Transform) return; //Transform component는 제거될수 없음 ( 필수)
                       
            if(_components.Contains(component)) //유효한 컴포넌트 타입인지 확인
            {
                IsActive = false;
    ...
    abstract class MSEntity : ViewModelBase
    {
        ...
        {
            var value = getProperty(objects.First());
            return objects.Skip(1).Any(x => !getProperty(x)) ? (bool?)null : value;
        }
