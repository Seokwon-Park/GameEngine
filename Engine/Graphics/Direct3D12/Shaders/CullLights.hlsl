...
groupshared uint _lightFlagsOpaque[MaxLightsPerGroup];      // flags the lights in the title that are actually affecting pixels.
groupshared uint _spotlightStartOffset;                     
groupshared uint2 _opaqueLightIndex;                        // x for point lihgts and y for spotlights.
...
void CullLightsCS(ComputeShaderInput csIn)
{
    ...
    // 
    // For our rh coord sys, column-major projection matrices are;
    //
