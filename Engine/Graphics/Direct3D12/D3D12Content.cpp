...
namespace primal::graphics::d3d12::content
{
	...
	namespace render_item
	{
		...
		// buffer[1 .. n] = submesh_gpu_ids(n is the number of submeshes which must also eqaul the number of material ids).
		...
		void get_d3d12_render_item_ids(const frame_info& info, utl::vector<id::id_type>& d3d12_render_item_ids)
		{
			...
			primal::content::get_lod_offsets(frame_cache.geometry_ids.data(), frame_cache.thresholds.data(), count, frame_cache.lod_offsets);
