...
#pragma intrinsic(_BitScanForward)
		...
		pso_id create_pso(id::id_type material_id, D3D12_PRIMITIVE_TOPOLOGY primitive_topology, u32 elements_type)
		{
			...
			{ // Lock materials
				...
				for (u32 i{ 0 }; i < shader_type::count; ++i)
				{
					if (flags & (1 << i))
					{
						// NOTE: each type of vertex may have keys that are generated from different properties of the submesh or material.
						//		 At the moments, we only have different kinds of vertex shaders depending on elements_type
	...
	namespace render_item
	{
		...
		// buffer[1 .. n] = submesh_gpu_ids(n is the number of low-level render item ids which must also eqaul the number of submeshes/material ids).
						...
						u32 material_count, const id::id_type* const material_ids)
		{
			...
			// NOTE: the lit of ids starts with geometry id and ends with an invalid id to mark the end of the list.
