	...
	namespace
	{
		...
		void 		fill_per_object_data(constant_buffer& cbuffer, const d3d12_frame_info& d3d12_info)
		{
			...
			for (u32 i{ 0 }; i < render_items_count; ++i)
			{
				if (current_entity_id != cache.entity_ids[i])
				{
					...
					XMMATRIX mvp{ XMMatrixMultiply(world,d3d12_info.camera->view_projection()) };
		...
		void set_root_parameters(id3d12_graphics_command_list*const cmd_list,u32 cache_index)
		{
			...
			case material_type::opaque:
			{
				...
				cmd_list->SetGraphicsRootShaderResourceView(params::per_object_data, cache.per_object_data[cache_index]);
			}
				break;
			}
