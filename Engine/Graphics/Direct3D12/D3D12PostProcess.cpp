...
namespace primal::graphics::d3d12::fx
{
	namespace 
	{
		...
		{	
			...
			assert(fx_root_sig);
			NAME_D3D12_OBJECT(fx_root_sig, L"Post-proecess FX Root signature");
	...
	void post_process(id3d12_graphics_command_list* cmd_list, D3D12_CPU_DESCRIPTOR_HANDLE target_rtv)
	{
		...
		// NOTE: we don't need to clear the render target, because each pixel will be overwritten by pixels from gapss main buffer.
