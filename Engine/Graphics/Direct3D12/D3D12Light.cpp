...
#define CONSTEXPR constexpr
		...
		public:
			...
			{
				// Update dircetion for non-cullable lights
			...
			constexpr void enable(light_id id, bool is_enabled)
			{
				...
				// NOTE: dirty_bits is goint to be set by swap_cullable_lights, so we don't set it here.
				if (is_enabled)
				{
					if (data_index > count)
					{
		...
		private:
			...
			CONSTEXPR void add_light_culling_info(const light_init_info& info, u32 index)
			{
				...
				{
#if USE_BOUNDING_SPHERES
					_culling_info[index].CosPenumbra = params.CosPenumbra;
#else
					_culling_info[index].ConeRadius = calculate_cone_radius(params.Range, params.CosPenumbra);
#endif
				}
			}
			...
			u8												_something_is_dirty{ 0 };  // flag is set if any cullable lights where changed.
